...
Every time this code is run the numbers will change, since these are random draws. In order to take random draws that do not change---and to use numbers that are therefore suitable for reproducible data analysis---we must set a random seed using the `set.seed()` function before running any random process.  The argument to `set.seed()` is a number.  It doesn't matter which number. As a general rule, **always set the seed.**
...
Average profit is over \$11,000 per month. But the minimum is actually a loss of almost \$9,000! What is the probability that Sanjay's monthly profit would be below his threshold of \$5000?
...
Sanjay should therefore expect to make a monthly profit of less than \$5000 about  30% of the time, even as his average profit is substantially above that.

Should he open the restaurant? That is his decision. But simulation modeling should make his decision  more informed.
